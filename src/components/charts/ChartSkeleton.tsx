...
interface ChartSkeletonProps {
  theme: ThemeMode;
  type?: 'sankey' | 'bar' | 'tree' | 'scatter' | 'radar';
}
    ...
    switch (type) {
      ...
      case 'radar':
        return (
          <div className="space-y-3">
            {/* 雷达图骨架 */}
            <div className={`h-64 rounded ${bgLightClass} flex items-center justify-center`}>
              <div className={`w-48 h-48 rounded-full border-2 ${isDark ? 'border-white/20' : 'border-black/20'} relative`}>
                {/* 同心圆 */}
                <div className={`absolute inset-4 rounded-full border ${isDark ? 'border-white/10' : 'border-black/10'}`} />
                <div className={`absolute inset-8 rounded-full border ${isDark ? 'border-white/10' : 'border-black/10'}`} />
                <div className={`absolute inset-12 rounded-full border ${isDark ? 'border-white/10' : 'border-black/10'}`} />
                {/* 数据点 */}

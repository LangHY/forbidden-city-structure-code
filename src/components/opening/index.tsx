...
function Opening({ initialTheme = 'dark' }: OpeningProps) {
  ...
  // 视差偏移：过渡区结束（300vh）后，Router 以 40% 速度上移
  const parallaxY = useMemo(() => {
    const transitionEnd = window.innerHeight * 3;
    const extra = Math.max(0, scrollY - transitionEnd);
    return extra * 0.4;
  }, [scrollY]);
  ...
  return (
    <>
      ...
      {/* 滚动占位空间：Opening → Router 过渡 */}
      <div className="h-[200vh] bg-white" />
    </ScrollProgressProvider>

    {/* Router 浏览区 + About 自然滚入 */}
